...
export default function ChatworkParser({ onImport }) {
  ...
  const handleOneClick = async () => {
    ...
    try {
      ...
      if (results.length === 0) {
        setResultMsg(
          `${messages.length}件のメッセージを確認しましたが、対象キーワードは見つかりませんでした`
        );
        setFetching(false);
        return;
      }
      ...
      onImport(newTasks);
      setResultMsg(
        `✅ ${results.length}件のタスクを追加しました！（${messages.length}件のメッセージから）`
      );
    } catch (err) {
    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-white rounded-xl p-4 mb-3 border-[1.5px] border-card-border">
        ...
        <div className="text-xs text-gray-400 mb-3 bg-[#FFF8F5] rounded-lg p-2 px-3 border border-[#FFE0D0] leading-relaxed">
          📌 ボタン1つでChatworkの最新メッセージからキーワードを検出し、タスクボードに自動追加します。
        </div>
        <button
